...
class MasterMachine:
    ...
    def Close(self):
        '''主控机关机'''
        self.__workMode=W_CLOSED
    def SlaveStart(self,roomId:int):
        '''房间号为roomID的从控机开机'''
    ...
    def SlaveClose(self,roomId:int):
        if roomId not in self.__roomList:
            raise RuntimeError('房间不存在')
        else:
    ...
    def SetDefaultParam(self,lowTemp:int,highTemp:int,minSpeed:int,maxSpeed:int,defaultSpeed:int,feeRate:dict):
        '''更改从控机默认参数'''
    ...
    def SetSlaveParam(self,slaveNum:int,tMode:int,wMode:int,sp:int):
        '''设置slaveNum号从控机的工作参数'''
